...
NUM_VEHICLES = 3;
RLENGTH = 2*pi*L;
HDES = (RLENGTH - NUM_VEHICLES*d)/NUM_VEHICLES;
HSAFE = 10;
...
for j = 1:1:N % Time Loop
    for i  = 1:1:n % Loop for number of cars
        % Find the position of the lead car
        if i == n  % if the car is lead car
            xl = x(1);
        else
            xl = x(i+1);
        end
        % Error: current headway - (desired headway + safety distance)
        err = s(i) - (HDES + HSAFE);
        cmd = Kp*err;
        dvdt(i) = validate_accel(s(i), dx(i), cmd, dt, HSAFE, VMAX, ubounds);
        if dx(i) < 0
